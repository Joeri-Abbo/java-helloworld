...
import com.joeriabbo.helloworld.Routes.Ping;
...
class PingTests {
    ...
    public void testHelloDefaultResponse() throws Exception {
        MvcResult mvcResult = mockMvc.perform(MockMvcRequestBuilders.get("/ping")).andExpect(MockMvcResultMatchers.status().isOk()).andReturn();
        Assertions.assertEquals(mvcResult.getResponse().getStatus(), 200);
        Assertions.assertEquals(mvcResult.getResponse().getContentAsString(), "Pong");
    }
}
